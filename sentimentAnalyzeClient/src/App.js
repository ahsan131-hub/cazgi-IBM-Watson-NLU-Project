...
class App extends React.Component {
  ...
  sendForSentimentAnalysis = () => {
    ...
    ret.then((response) => {
      ...
      console.log("response as " + response.data.document.score);
      this.setState({ sentimentOutput: response.data.document.label });
      let output = response.data.document.label;
      if (response.data.document.label === "positive") {
        output = (
          <div style={{ color: "green", fontSize: 20 }}>
            {response.data.document.score}
          </div>
        );
      } else if (response.data.document.label === "negative") {
        output = (
  ...
  sendForEmotionAnalysis = () => {
    ...
    ret.then((response) => {
      console.log(response.data.document.emotion);

      this.setState({
        sentimentOutput: <EmotionTable emotions={response.data.document.emotion} />,
      });
    });
  };
